# README assets

Place marketing screenshots here for the README files:

| File | Suggested content |
|------|-------------------|
| `logo.png` | Square logo (~256×256) |
| `demo-before-after.png` | Reference photo vs lit 3D subject |
| `demo-dual-gel.png` | Blue + magenta gel lighting example |
| `demo-panel.png` | RollLux N-panel screenshot |
| `demo-workflow.gif` | Load → Generate → Tune (short GIF) |

Paths are referenced from `README.md`, `README_zh-CN.md`, and `README_ja.md`.
