...
| 🔧 **单灯编辑** | 开关、改色、能量、柔化、删除 |
| 🔒 **锁定灯光颜色** | 重新生成或切换预设时保持当前调色板 |
| 🪟 **浮层参考** | 视口固定参考图（透明度、大小、角落） |
| 📂 **打开图片** | 从磁盘加载参考图 |
| 🌐 **三语界面** | 跟随 Blender 语言偏好（English / 中文 / 日本語） |
...
1. 从 [Releases](https://github.com/AIGODLIKE/RollLux/releases/latest) 下载 **`rolllux-5.5.5.zip`**。
2. Blender → **编辑 → 偏好设置 → 获取扩展 → ▼ → 从磁盘安装…**
3. 选择 zip → 启用 **RollLux**。

自 5.0 起的变更见 [CHANGELOG.md](CHANGELOG.md)。
...
```bash
cd rolllux
py dev/gen_assets_main.py   # 可选：重新生成缩略图与参考图
py build.py                 # 输出 -> ../dist/rolllux-<version>.zip
py build_marketplace.py     # 输出 -> ../dist/rolllux-<version>-marketplace.zip（Extension Platform）
```
...
## 🤖 MCP — 用 Cursor / Codex 驱动打光

在对话里直接控制 RollLux，例如：*「给 Suzanne 打 portrait 光」*、*用 golden hour 参考给选中物体打 cinematic 光*、*给当前模型上 Arcane 风格*。

> **仅完整版** — 请安装 [GitHub Releases](https://github.com/AIGODLIKE/RollLux/releases) 的 `rolllux-*.zip`。Extension Platform 包（*-marketplace.zip*）不含 MCP 桥接。

```
Cursor / Codex  ──stdio──►  rolllux_mcp  ──TCP :9886──►  Blender（RollLux 面板 ▶ Start MCP）
```

### 准备

1. **Blender** — 启用 RollLux → **N** 面板 → **▶ Start MCP**（端口 **9886**）。
...
若只克隆了 `rolllux` 仓库，复制 [`.cursor/mcp.json.example`](.cursor/mcp.json.example)，把 `cwd` 改为本机 `rolllux/mcp` 的绝对路径。  
然后在 **Cursor 设置 → MCP** 中启用 **rolllux** 并重启 Cursor。

### OpenAI Codex

写入 `~/.codex/config.toml`（或信任项目后使用 `rolllux/.codex/config.toml`）：

```toml
[mcp_servers.rolllux]
command = "py"
args = ["-m", "rolllux_mcp"]
cwd = "C:/你的路径/magic-world/rolllux/mcp"
enabled = true
...
或执行 `codex mcp add rolllux -- py -m rolllux_mcp`（再补全 `cwd` 与环境变量）。  
在 Codex 会话中输入 **`/mcp`** 确认工具已加载。  
模板：[`.codex/config.toml.example`](.codex/config.toml.example)

### 示范对话

<details>
<summary><b>示范 1 — 人像（Cursor）</b></summary>

**你：** 检查 Blender 连接，列出网格，给 **Suzanne** 用 **portrait** 策略打 4 盏灯。
...
<details>
<summary><b>示范 2 — 参考预设（Codex）</b></summary>

**你：** 清掉 RollLux，用 **golden_hour** 参考给当前选中物体打 cinematic 光。
...
<details>
<summary><b>示范 3 — 风格化（任意客户端）</b></summary>

先在 Blender 里选中目标网格，然后：

**你：** 给当前物体上 **Arcane** 青绿 + 琥珀色风格光。
...
风格工具（`light_jinx`、`light_doraemon`、`light_cyberpunk` 等）作用于 **活动物体**；指定名称请用 `light_object(object_name=…)`。

</details>

| 工具 | 作用 |
|------|------|
| `check_blender` | 检测连接与 RollLux 是否安装 |
| `list_scene_objects` | 列出场景网格名称 |
| `light_object` | 给 **指定名称** 物体打光 |
| `light_selection` | 给 **当前选中** 打光 |
| `clear_rolllux_lighting` | 清除灯光 rig |
| `light_arcane`、`light_jinx` 等 | 风格化（活动物体） |
